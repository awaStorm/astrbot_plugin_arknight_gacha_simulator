...
│   ├── plugin_migrate.py         # 旧版本数据迁移（升级时自动执行）
│   └── db_manager.py             # SQLite 数据持久化
├── tools/                        # 离线数据处理 / 本地调试工具
...
- 修复首次安装时 `processed/`、`raw/` 等运行时数据子目录不存在，导致数据拉取脚本写入失败（`[Errno 2] No such file or directory`）、卡池数据始终拉不下来、抽卡引擎无法初始化的问题。
  - 插件入口在注入插件专属数据目录后，统一创建 `cache/`、`raw/`、`processed/`、`fonts/` 子目录。
  - `tools/` 下各数据脚本（`fetch_characters` / `fetch_gacha_wikitext` / `clean_gacha_pools` / `post_process_pools`）在读写前也会各自确保目录存在，以便脱离插件独立运行。
- 优化引擎初始化的日志：首次运行数据尚未就绪时不再输出 `error` 级别的堆栈（属预期流程，自动更新完成后会重建引擎），仅在数据已存在却加载失败时才报错。

**修复：图片元素缺失、抽卡卡顿与若干健壮性问题**

- **修复图片中人物、头像与图标全部缺失**：`set_data_dir()` 注入插件专属数据目录时遗漏了 4 个缓存子目录常量（`PROF_DIR` / `PROFESSION_LABELED_DIR` / `PORTRAIT_DIR` / `ELITE1_ART_DIR`），使「写入目录」与「读取目录」永久错位。**在 AstrBot 中运行时头像、职业图标、带文字职业图标与立绘会全部读不到**；而本地调试脚本不经过该路径，因此不易察觉。已补全常量重算。
- **修复单抽图没有干员立绘**：立绘按画质档位写入 `elite1_art/<档位>/` 子目录，读取时却只在根目录查找，导致始终 miss 并静默跳过绘制。现在由渲染器把当前档位目录注入合成器，并保留「根目录 → 各档位子目录」的兜底查找。
- **修复图片合成阻塞事件循环**：单抽 / 十连的合成是同步 CPU 操作（实测单抽约 470 ms、十连约 870 ms），会冻结整个 AstrBot 进程的所有用户与其它插件。现已改为在线程中执行，渲染期间事件循环可正常调度。
- **修复首次安装后指令可能长时间无响应**：抽卡引擎初始化会在事件循环中**同步拉起子进程**（`timeout=60`），最坏把 AstrBot 卡住 60 秒。现在初始化移到线程，且指令路径不再触发即时生成（交由后台完成）。
- **修复卡池数据文件损坏会导致插件永久不可用**：`cleaned_pools_final.json` / `active_pools.json` 读取异常会穿透初始化流程，使插件一直停在「数据未加载」且不自愈。现已捕获并降级为空数据，等待重新生成。
- **修复已结束的卡池仍可继续抽**：内存中的卡池列表不会随时间自动刷新，而旧池结束不会触发数据更新。现在每次使用前按 `time_end` 剔除已结束的卡池。
- **修复跨年欢庆池「首次 6★ 必定未持有」规则未生效**：`owned_characters` 参数始终传空，使该保护分支永远不成立，现已接入用户已持有干员列表。
- 自动更新器的启动任务改为**持有引用**，避免其在网络等待期间被垃圾回收，导致首次数据拉取静默失败、后续定时更新也不再启动。
- **数据尚未就绪时主动拉取**：首次安装等场景下，指令不再只返回「未就绪」，而会在后台触发一次恢复（本地重建 → 必要时联网拉取，自带去重），并返回明确提示。
- 立绘解码结果加入 **LRU 缓存**（上限 12 张），连续抽卡时无需重复解码同一张立绘。
